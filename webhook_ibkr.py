...
# # Global variables
# total_contracts = 0
# ib = None  # Initialize IB connection once

# # Locks for shared resources
# contracts_lock = threading.Lock()
# ib_lock = threading.Lock()
# order_lock = threading.Lock()  # Lock for order placement
...
#             try:
#                 ib = IB()
#                 ib.connect('127.0.0.1', 7497, clientId=111, timeout=15)
#                 print("Connected to IBKR.")
#             except Exception as e:
#                 print(f"API connection failed: {e}")
#                 return False
...
# # Initialize IB connection at startup
# connect_ibkr()
...
#     # Place orders with lock to prevent rapid submissions
#     with order_lock:
#         if bracket is not None:
#             bracket[0].outsideRth = True
#             ib.placeOrder(contract, bracket[0])
#             print(f"Placed bracket order: {signal.upper()} (Parent orderId: {bracket[0].orderId})")
#             if bracket.stopLoss is not None:
#                 bracket[2].outsideRth = True
#                 ib.placeOrder(contract, bracket[2])
#                 print(f"Stop @ {stop_price} (Stop orderId: {bracket[2].orderId})")
#             # if bracket.takeProfit is not None:
#             #     bracket[1].outsideRth = True
#             #     ib.placeOrder(contract, bracket[1])
#             #     print(f"Take-Profit @ {bracket[1].lmtPrice} (TP orderId: {bracket[1].orderId})")
#             ib.sleep(0.5)  # Delay to ensure IB API processes IDs
#         elif market is not None:
#             ib.placeOrder(contract, market)
#             print(f"Placed market order: {signal.upper()} (orderId: {market.orderId})")
#             open_orders = ib.openOrders()
#             # Filter stop loss orders
#             stop_orders = [o for o in open_orders if o.orderType == 'STP']
#             if stop_orders:
...
#             else:
#                 print("No open stop loss orders found.")
#             ib.sleep(0.5)  # Delay for stability
#         else:
#             print("No order placed due to invalid conditions.")
...
#     # Run place_order function directly
#     place_order(signal)
